...
import com.example.input.PcProductLog;
...
public class PcProductGenerateData {
    ...
    static{
        for(int i=0;i<5;i++){
            long scantime = genernateDateData(new Date());
    ...
    private static PcProductLog genernateData(){
        ...
        if(randomSelect > 1){
            ...
            pcProductLog.setMacAdress(macAdresses[random.nextInt(macAdresses.length)]);
        }
        pcProductLog.setProductTypeId(productTypeIs[random.nextInt(productTypeIs.length)]);
        pcProductLog.setProductId(productIds[random.nextInt(productIds.length)]);
        pcProductLog.setPindaoId(pindaoIds[random.nextInt(pindaoIds.length)]);
        ...
        pcProductLog.setScantime(scanTime);
        pcProductLog.setJumpTime(jumpTime);

        pcProductLog.setCookie(cookiees[random.nextInt(cookiees.length)]);
        pcProductLog.setRemoteName(remoteNamees[random.nextInt(remoteNamees.length)]);
        pcProductLog.setDeviceType(deviceTypees[random.nextInt(deviceTypees.length)]);
        pcProductLog.setOs(oses[random.nextInt(oses.length)]);
        pcProductLog.setBrower(broweres[random.nextInt(broweres.length)]);
        pcProductLog.setResolution(resolutiones[random.nextInt(resolutiones.length)]);
        pcProductLog.setSourceInfo(sourceInfoes[random.nextInt(sourceInfoes.length)]);
        pcProductLog.setSourceType(sourceTypees[random.nextInt(sourceTypees.length)]);
        pcProductLog.setSrcDomain(srcDomaines[random.nextInt(srcDomaines.length)]);
        pcProductLog.setLogType(LogType.PCPRODUCT);
        return  pcProductLog;
    ...
    public static void main(String[] args) {
        for (int i = 0; i < 50; i++) {
            ...
            System.out.println(JSONObject.toJSON(pcProductLog));
            postHttpMethod("http://127.0.0.1:8081/collectAppData", JSONObject.toJSONString(pcProductLog));
        }
    }
}
